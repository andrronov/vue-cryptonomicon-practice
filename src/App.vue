      ...
      <section>
        <div class="flex">
          <div class="max-w-xs">
            ...
            >
              <span
              v-for="(tik, index) in autoComplete" :key="index"
              @click="addFromComplete(tik.Symbol)"
...
watch(ticker, async() => {
  ...
  autoComplete.value = dataValues.filter(tik => tik.Symbol.substr(0, ticker.value.length).toUpperCase() == ticker.value.toUpperCase()).splice(0, 5)  
})

function addFromComplete(tick){
  ticker.value = tick
  add()
}
function add() {
  ...
  tickers.value.forEach(tik => {if(tik.name === currentTicker.name) throw error.value = true})
  tickers.value.push(currentTicker);
  saveToLocalStorage();
  subscribeToTickerUpdate(currentTicker.name);
  ticker.value = "";
}

function subscribeToTickerUpdate(tickerName) {
  setInterval(async () => {
    const f = await fetch(
